...
binary_artifact="$binary_build_path/bin/orcus"
...
usage() {
  ...
  echo "-N do not build; use existing ./buld/bin/orcus binary"
...
setup_build_path() {
  ...
  build_path=$(mktemp -d $release_base_path/orcusXXXXXX) || fail "Unable to 'mktemp -d $release_base_path/orcusXXXXXX'"
...
build_binary() {
  ...
  find $binary_artifact -type f || fail "Failed to generate orcus binary"
}

copy_binary_artifacts() {
  build_path="$1"
  cp $binary_artifact "$build_path/orcus/usr/bin/" && debug "binary copied to $build_path/orcus/usr/bin/" || fail "Failed to copy orcus binary to $build_path/orcus/usr/bin/"
}
...
setup_artifact_paths() {
  ...
  mkdir -p $build_path/orcus
  mkdir -p $build_path/orcus/usr/bin/
  mkdir -p $build_path/orcus/etc/systemd/system
  mkdir -p $build_path/orcus/var/log/orcus
...
function copy_resource_artifacts() {
  ...
  cd  $basedir
  gofmt -s -w  .
  rsync -qa ./conf/orcus.cnf $build_path/orcus/etc/
  cp etc/systemd/orcus.service $build_path/orcus/etc/systemd/system/orcus.service
...
package_linux() {
  ...
  [ $do_tar -eq 1 ] && tar -C $build_path/orcus -czf $release_base_path/orcus-"${RELEASE_VERSION}"-$target-amd64.tar.gz ./

  debug "Creating Distro full packages"
  [ $do_rpm -eq 1 ] && fpm -v "${RELEASE_VERSION}" --epoch 1 -f -s dir -n orcus -m max-fedotov --description "Orchestrator to consul synchronization tool" --url "https://github.com/MaxFedotov/orcus" --vendor "Max Fedotov" --license "Apache 2.0" -C $build_path/orcus --prefix=/ --config-files /etc/orcus.cnf --rpm-os linux --before-install $build_path/tmp/pre-install --rpm-attr 744,orcus,orcus:/var/log/orcus -t rpm .
  [ $do_deb -eq 1 ] && fpm -v "${RELEASE_VERSION}" --epoch 1 -f -s dir -n orcus -m max-fedotov --description "Orchestrator to consul synchronization tool" --url "https://github.com/MaxFedotov/orcus" --vendor "Max Fedotov" --license "Apache 2.0" -C $build_path/orcus --prefix=/ --config-files /etc/orcus.cnf --before-install $build_path/tmp/pre-install -t deb --deb-no-default-config-files .
...
package_darwin() {
  ...
  tar -C $build_path/orcus -czf $release_base_path/orcus-"${RELEASE_VERSION}"-$target-amd64.tar.gz ./
}
...
debug "orcus build done; exit status is $?"
